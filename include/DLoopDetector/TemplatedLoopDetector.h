...
public:
  ...
  bool detectLoop(const std::vector<cv::KeyPoint> &keys, 
    ...
    DetectionResult &match, double &timestamp_, map<DBoW2::EntryId, double> &entry2Time_);
...
private:
    EntryId mnlastloop;   // last loop entry id, we don't need continuous loop result
    EntryId mnthreshold;  // this is threshold , set 20
  ...
  DetectionResult &match, double &timestamp_, map<DBoW2::EntryId, double> &entry2Time_)
{
  ...
  // added by doris
  if(entry2Time_.find(entry_id) == entry2Time_.end())
  {
    entry2Time_[entry_id] = timestamp_;
  }
  
  
//  cout << "entry_id : " << entry_id << endl;
  ...
  if(m_params.geom_check == GEOM_DI) {
    m_database->getVocabulary()->transform(descriptors, bowvec, featvec,     //传进来的图的向量
                                           m_params.di_levels);
  }
  else
    m_database->getVocabulary()->transform(descriptors, bowvec);

  if((int)entry_id <= m_params.dislocal || (entry_id - mnlastloop < mnthreshold) )   // this means if seqs less than dislocal, there will no detection
  {
  ...
  {
    int max_id = (int)entry_id - m_params.dislocal;   //也就是说只从离该帧有m_params.dislocal个距离的帧里面选最近的。比如当前帧为100，那么就从0-80里面选最近的图像，此处m_params.dislocal=20

    QueryResults qret;

    //qret包含了分数最大的图片的id和分数
    m_database->query(bowvec, qret, m_params.max_db_results, max_id);    //会用当前的图片的词向量和 m_database中已经存储的图片的词向量进行比较打分，得出前m_params.max_db_result 个分数最大的id和分数

    //add 添加了词向量和该图片的id（id的添加是通过类的内部成员m_nentries完成的）  因为query会遍历m_database，所以应该有 delete的操作来减少运算量。
    // update database
    m_database->add(bowvec, featvec); // returns entry_id     //将当前图片的词向量加入m_database。内部会进行m_ifile的更新，会存储这个图片的id以及词向量，方便query进行操作.
    ...
    if(!qret.empty())
    {
      ...
      if(m_params.use_nss)
      {
        ns_factor = m_database->getVocabulary()->score(bowvec, m_last_bowvec);  //先和前一帧的词向量进行比较
      }
      ...
      if(!m_params.use_nss || ns_factor >= m_params.min_nss_factor)
      {
        ...
        if(!qret.empty())
        {
          ...
          if(!islands.empty())
          {
            ...
            if(getConsistentEntries() > m_params.k)   //当连续的island有交集时，m_window.nentries就不断增加，当m_window.nentries>m_params.k时，就认为通过了时间检测
            {
              ...
              if(m_params.geom_check == GEOM_DI)
              {
                ...
                //找到本图的descriptor和它所找到的island的最好岛民的descriptors，根据两个图的descriptors，找到两张图最可能的匹配点，然后根据最可能的匹配点，去找是否能找到满足一定条件的fundamentmatrix。如果找到了，就认为满足了空间约束。
  ...
  (QueryResults &q, vector<tIsland> &islands) const
{
  ...
  if(q.size() == 1)   //只有一个q的时候，也就是只有一个岛民，也就只会有一个岛，一个岛只有一个岛民。
  {
    ...
    islands.back().best_score = q[0].Score;
      ...
      //当q的数量很多时，即岛民很多， 不同的岛民按照距离（entry_id的间隔。）分布在不同的岛上，
      // 不同的岛就组成了岛群。当两个岛民间距小于 m_params.max_intragroup_gap时，认为他们是同一个岛上的岛民。
  else if(!q.empty())
  {
    // sort query results in ascending order of ids
    std::sort(q.begin(), q.end(), Result::ltId);   //q是按照 entry_id排序的
    ...
    for(unsigned int idx = 1; dit != q.end(); ++dit, ++idx)
    {
      ...
      else
      {
        ...
        if(length >= m_params.min_matches_per_group)    //岛上岛民的最小数量
        {
  ...
  (const tIsland &matched_island, EntryId entry_id)
{
  ...
  if(m_window.nentries == 0 || int(entry_id - m_window.last_query_id)   //保证当前的entry_id与  m_window.last_query_id是相邻的
    > m_params.max_distance_between_queries)      //m_params.max_distance_between_queries = 2
  {
    m_window.nentries = 1;
  }
  else
  {
    ...
    bool fit = (b1 <= a1 && a1 <= b2) || (a1 <= b1 && b1 <= a2); //判断两个不同的island是否有交集， 如果有交集，说明这个根据分数最高得到的island可能真的是个回环。
    ...
    if(!fit)
    {
      ...
      fit = (gap <= m_params.max_distance_between_groups);   //当两个island之间的距离小于m_params.max_distance_between_groups时，也可以认为两个island有交集
    }
    
    if(fit) m_window.nentries++;     //当 m_window.nentries大于一定的数量时候，就可以认为有回环了
  ...
  vector<unsigned int> &i_match_A, vector<unsigned int> &i_match_B) const 
{
  ...
  for(ait = i_A.begin(); ait != i_A.end(); ++ait, ++i)
  {
    ...
    if(best_dist_1 / best_dist_2 <= m_params.max_neighbor_ratio)  // 1nn 2nn 算法中的阈值
    {
